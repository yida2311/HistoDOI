...
def save_model(model, path, scores_node, scores_edge, task_name, epoch, best_pred):
...
def write_log(f_log, train_scores_node, train_scores_edge, val_scores_node, val_scores_edge, epoch):
    log = ""
...
class Trainer():
    def __init__(self, criterion1, criterion2, optimizer, n_class, alpha=1.0):
    ...
    def train(self, sample, model):
        model.train()
        sample = sample.cuda()
        label_node = sample.y.cuda()
        label_edge = sample.label_edge.cuda()
