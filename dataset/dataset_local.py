...
class OralDatasetLocal(Dataset):
    ...
    def __init__(self,
                ...
                transform=None):
        super(OralDatasetLocal, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        tile_id = info['slide_id'] + '_' + str(info['row']) + '_' + str(info['col'])
        img_path = os.path.join(os.path.join(os.path.join(self.img_dir, info['slide_id']), tile_id), info['image_id'])
        img = cv2_image_loader(img_path)
        sample = {}
        
        if self.transform:
            if self.label:
                mask_path =  os.path.join(os.path.join(os.path.join(self.mask_dir, info['slide_id']), tile_id), info['image_id'])
...
class OralSlideLocal(Dataset):
    ...
    def __init__(self,
        ...
        """
        super(OralSlideLocal, self).__init__()
    ...
    def get_patches_from_index(self, index):
        ...
        self.tiles = [c for c in os.listdir(slide_dir) if not c.endswith('.png')]
        self.samples = []
        for tile in self.tiles:
            self.samples += os.listdir(os.path.join(slide_dir, tile))
    ...
    def __getitem__(self, index):
        ...
        ver, col, x, y = self._parse_patch_name(patch)
        tile = self.slide + '_' + str(ver) + '_' + str(col)
        img_path = os.path.join(os.path.join(os.path.join(self.img_dir, self.slide), tile), patch)
        img = cv2_image_loader(img_path)
        sample = {}

        if self.transform:
            if self.label:
                mask_path = os.path.join(os.path.join(os.path.join(self.slide_mask_dir, self.slide), tile), patch)
                mask = cv2_mask_loader(mask_path)
                sample = self.transform(image=img, mask=mask)
            else:
                sample = self.transform(image=img)
        
        sample['coord'] = (ver, col, x, y)
        return sample
